...
typedef enum comando { r_stampa = 1, r_ordina_data, r_ordina_cod, r_ordina_part, r_ordina_arr, r_ricerca, r_aggiungi, r_exit } comando;
...
typedef struct corsa {
	char codice[MAXSTR], partenza[MAXSTR], destinazione[MAXSTR]; // TODO: chiedere se fare puntatori
	int ritardo;
	struct data data;
	struct ora ora_fine, ora_inizio;
} corsa;

int leggiFile(const char* fn, corsa** d) {
	int righe, i, retVal;
	corsa* dati;
	...
	dati = (corsa*)malloc(righe * sizeof(corsa));
...
int ricerca_lineare(corsa* dati, int N, const char* src, corsa* ris[]) {
...
int ricerca_dicotomica(corsa** dati, int N, const char* src, corsa* ris[]) {
...
int main() {
	corsa *dati, ** ord_date, ** ord_cod, ** ord_part, ** ord_arr, ** ris;
	int righe = leggiFile("log.txt", &dati), i, is_ord_partenza = 0, n;
	comando scelta;
	char src[MAXSTR], filename[50];

	if (righe == -1)
	{
		printf("File di log non presente\n");
		exit(1);
	}

	ord_date = (corsa**)malloc(righe * sizeof(corsa*));
	ord_cod = (corsa**)malloc(righe * sizeof(corsa*));
	ord_part = (corsa**)malloc(righe * sizeof(corsa*));
	ord_arr = (corsa**)malloc(righe * sizeof(corsa*));
	ris = (corsa**)malloc(righe * sizeof(corsa*));

	for (i = 0; i < righe; i++)
		ord_date[i] = ord_cod[i] = ord_part[i] = ord_arr[i] = &dati[i];
	...
	do
	{
		...
		case r_ordina_data:
			bubbleSort_date(ord_date, righe);

			for (i = 0; i < righe; i++)
				stampaCorsa(ord_date[i], stdout);
			break;

		case r_ordina_cod:
			bubbleSort_tratta(ord_cod, righe);

			for (i = 0; i < righe; i++)
				stampaCorsa(ord_cod[i], stdout);
			break;

		case r_ordina_part:
			bubbleSort_partenza(ord_part, righe);
			is_ord_partenza = 1;

			for (i = 0; i < righe; i++)
				stampaCorsa(ord_part[i], stdout);
			break;

		case r_ordina_arr:
			bubbleSort_arrivo(ord_arr, righe);

			for (i = 0; i < righe; i++)
				stampaCorsa(ord_arr[i], stdout);
			break;
		...
		case r_ricerca:
		{
			...
			scanf("%s", src);
			if (is_ord_partenza)
				n = ricerca_dicotomica(ord_part, righe, src, ris);
			else
				n = ricerca_lineare(dati, righe, src, ris);

			for (i = 0; i < n; i++)
				stampaCorsa(ris[i], stdout);
