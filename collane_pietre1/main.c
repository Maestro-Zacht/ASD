...
int check_pietra(collana* c, enum pietra inserire) {
...
collana add_pietra(elenco_pietre_rimanenti* elenco, collana* collana_attuale) {
	int i;
	collana best, tmp;
...
collana make_collana(elenco_pietre_rimanenti* elenco, collana* c) {
...
int main() {
	int N, i, ret_val, j;
	elenco_pietre_rimanenti elenco;
	collana c, res;
	...
	for (i = 0; i < N; i++) {
		ret_val = fscanf(infile, "%d %d %d %d", &elenco.elenco[0], &elenco.elenco[1], &elenco.elenco[3], &elenco.elenco[2]);
		if (ret_val != 4) {
			printf("Input da file non valido\n");
			exit(-1);
		}
		...
		res = make_collana(&elenco, &c);
		
		printf("Lunghezza della collana %d: %d\n", i + 1, res.lunghezza);
